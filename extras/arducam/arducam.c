...
uint8_t arducam(sensor_model_t model)
{
	...
	case smOV5642:
		{
			...
			else
			{
				...
				arducam_i2c_ord_read(0x3818,&reg_val);
...
void arducam_burst_read_start(void)
{
	arducam_spi_transfer_start();
}

void arducam_burst_read_stop(void)
{
	arducam_spi_transfer_stop();
}

void arducam_burst_read_fifo(uint8_t *dummy_buf, uint8_t *buf, uint32_t size)
{
	arducam_spi_transfer(dummy_buf, buf, size);
}
